...
class CopyThread(AbstractMDGThread):
    ...
    def run(self) -> None:
        ...
        logging.info('Starting analysing mods for jar in jar.')
        ...
        use_cached_deobf = []
        if cache_enabled:
            # remove mods from decompiled_mods which not in current mods_list
            if os.path.exists(PathUtils.DECOMPILED_MODS_PATH):
                for mod in os.listdir(PathUtils.DECOMPILED_MODS_PATH):
                    ...
                    if (mod_name in mods_list and ((decompiled_with_deobf and deobf_enabled) or
